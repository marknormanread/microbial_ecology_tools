...
)
{
  ...
  if (! is.na(datasave_prefix)) {
    dir.create(datasave_prefix, showWarnings = FALSE)  # Create if it doesn't already exist. 
    write.csv(group_differences_ks, paste0(datasave_prefix, '/group_differences_KS.csv'), quote = FALSE)
    write.csv(group_differences_p, paste0(datasave_prefix, '/group_differences_pvalue.csv'), quote = FALSE)
  }
